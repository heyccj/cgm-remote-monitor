...
function init (ctx) {
  ...
  function updateReservoir (prefs, result) {
    if (result.reservoir) {
      ...
      result.reservoir.display = 'units' + result.reservoir.value.toPrecision(3) + 'U endunits';
  ...
  function updateBattery (type, prefs, result) {
    if (result.battery) {
      ...
      result.battery.display = 'batt' + result.battery.value + type + 'endbatt';
  ...
  function updateStatus(pump, result) {
    if (pump.status) {
      ...
      status = 'status' + status + 'endstatus';
  ...
  function formatAgo (m, nowMills) {
    ...
    return translate('time' + '%1' + ago.shortLabel + (ago.shortLabel.length === 1 ? ' ago' : '') + 'endtime', { params: [(ago.value ? ago.value : '')]});
  }
