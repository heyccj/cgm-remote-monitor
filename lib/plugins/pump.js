...
function init (ctx) {
  ...
  function updateReservoir (prefs, result) {
    if (result.reservoir) {
      ...
      result.reservoir.display = 'x' + result.reservoir.value.toPrecision(3) + 'Ux';
  ...
  function updateBattery (type, prefs, result) {
    if (result.battery) {
      ...
      result.battery.display = 'y' + result.battery.value + type + 'y';
  ...
  function updateStatus(pump, result) {
    if (pump.status) {
      var status = 'z' + pump.status.status || 'normal' + 'z';
  ...
  function formatAgo (m, nowMills) {
    ...
    return translate('n' + '%1' + ago.shortLabel + (ago.shortLabel.length === 1 ? ' ago' : '') + 'n', { params: [(ago.value ? ago.value : '')]});
  }
