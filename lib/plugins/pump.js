...
function init (ctx) {
  ...
  function updateReservoir (prefs, result) {
    if (result.reservoir) {
      ...
      result.reservoir.display = '<div id=\"#units\">' + result.reservoir.value.toPrecision(3) + 'U<\/div>';
  ...
  function updateBattery (type, prefs, result) {
    if (result.battery) {
      ...
      result.battery.display = '<div id=\"#batt\">' + result.battery.value + type + '<\/div>';
  ...
  function updateStatus(pump, result) {
    if (pump.status) {
      var status = '<div id=\"#status\">' + pump.status.status || 'normal' + '<\/div>';
